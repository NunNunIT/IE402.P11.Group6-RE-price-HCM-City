...
export function CustomizeMap({
  ...
  title: string;
  locate: { lat: number; long: number; huyen: string };
  polygon: [number, number][];
}) {
  const { data } = useSWR(
    `/api/locations?sort=locate:${locate.lat},${locate.long}&district=${locate.huyen}&limit=24`,
    fetcher
  );
  ...
  return (
    <GISMap
      zoom={20}
      className="container"
      isShowDistrict={!polygon.length}
      center={locate}
      polygon={{ rings: polygon, title }}
      points={[
